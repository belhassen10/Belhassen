# -*- coding: utf-8 -*-
"""
Created on Fri Aug  5 16:48:59 2022
...
class ScrapeRssSpider(scrapy.Spider):
    name = 'le_360-rss'
    allowed_domains = ['https://fr.le360.ma/rss/']
    start_urls = ['https://fr.le360.ma/rss/']
    ...
    def closed(self ,reason):
        ...
        for i in range((len(items))):
            for p in ['title','author','pubDate','description','text']:
                if items[i][p]==None:
                    items[i][p]='cette colonne est vide'
        for i in range((len(items))):
             for p in ['title','pubDate','description']:
